...
const Services = () => {
  ...
  const { items: services = [], loading, error } = useSelector(state => state.service);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <p className="text-center text-gray-600 mb-6">
        Listed below is only a partial list of excavation and earth moving services we provide. If
        you don’t see the specific earth moving service you are looking for, contact us anyway.
      </p>
      ...
      <div className="flex flex-wrap -m-4">
        {displayServices.map(service => (
          <ServiceCard
            key={service._id}
            title={service.title}
            image={service.banners?.[0] || service.image} // Handle both static data formatting and backend formatting
            id={service._id}
          />
        ))}
      </div>
